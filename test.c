...
int main(int argc, char *argv[])
{
    ...
    printf("\n--- SPINLOCK BENCHMARK SUITE START ---\n"
           ...
           g_conf_spin_min, g_conf_spin_max, SETTLE_DELAY_MS, mlock_status, pin_desc);
     ...
     */
    double t_ttas = -1.0, t_pthread = -1.0, t_mcs = -1.0;
    int printed = 0;
    if (g_run_ttas) {
        t_ttas = run_benchmark("Custom TTAS Spinlock", task_spinlock_ttas);
        printed = 1;
    }
    if (g_run_pspin) {
        if (printed) {
        ...
        t_pthread = run_benchmark("POSIX Spinlock", task_pthread_spin);
        printed = 1;
    }
    if (g_run_mcs) {
        if (printed) {
        ...
        t_mcs = run_benchmark("Custom MCS Spinlock", task_spinlock_mcs);
    }

    const char *winner = NULL;
    double best = 1e300;
    if (g_run_ttas && t_ttas < best) {
        best = t_ttas;
        winner = "Custom TTAS Spinlock";
    }
    if (g_run_mcs && t_mcs < best) {
        best = t_mcs;
        winner = "Custom MCS Spinlock";
    }
    if (g_run_pspin && t_pthread < best) {
        best = t_pthread;
